#!/bin/bash
# repatch-recovery.sh — MANUAL DISASTER-RECOVERY variant of repatch.sh,
# meant to be run from a live/rescue environment (like the install USB)
# rather than from an already-patched, currently-booted SteamOS slot.
#
# Differences from the version this installs onto a working system:
#   - ROOTDEV / EFIDEV can be overridden via environment variables, in
#     case /dev/disk/by-partsets/<PARTSET>/... doesn't resolve correctly
...
#        bash repatch-recovery.sh A
#
# Everything else is identical to the on-device repatch.sh.
set -euo pipefail
...
cp -a "$UPDATE_WRAPPER_SRC" "$NEWROOT/usr/bin/steamos-update"
chmod 755 "$NEWROOT/usr/bin/steamos-update"
